...
contract CCIPTokenTransferer is CCIPReceiver, OwnerIsCreator {
    ...
    /// @notice Sends only text data to a receiver on the destination chain.
    /// @notice Pay for fees in LINK.
    ...
    function sendTextMessagePayLINK(
        ...
        external
        // onlyOwner // You can decide if this should be owner-only
        // onlyAllowlistedDestinationChain(_destinationChainSelector) // Optional security
        // validateReceiver(_receiver) // Optional security
        returns (bytes32 messageId)
    {
        // 1. **Build the message with an empty token array.**
        // This is the key difference: `tokenAmounts` is an empty array.
        Client.EVM2AnyMessage memory evm2AnyMessage = Client.EVM2AnyMessage({
            receiver: abi.encode(_receiver),
            data: abi.encode(_text),
            tokenAmounts: new Client.EVMTokenAmount[](0), // Empty array for no tokens
            extraArgs: Client._argsToBytes(
                Client.GenericExtraArgsV2({gasLimit: 200_000, allowOutOfOrderExecution: true})
            ),
            feeToken: address(s_linkToken) // Pay fees in LINK
        });

        IRouterClient router = IRouterClient(this.getRouter());

        // 2. **Calculate and pay the fee in LINK.**
        uint256 fees = router.getFee(_destinationChainSelector, evm2AnyMessage);
        ...
        // Approve the router to spend LINK for the fees.
        s_linkToken.approve(address(router), fees);
        
        // 3. **Send the message.**
        messageId = router.ccipSend(_destinationChainSelector, evm2AnyMessage);

        // Emit an event with zero values for the token transfer.
        emit MessageSent(
            messageId,
            _destinationChainSelector,
            _receiver,
            _text,
            address(0), // No token address
            0,          // No token amount
            address(s_linkToken),
            fees
        );
